...
nonisolated enum CentsFirstAmountInput {
    /// Display-only prefill. Callers must not write the result back to the model during synchronization.
    static func digits(for amount: Double?, currencyCode: String) -> String {
    ...
    static func amount(for digits: String, currencyCode: String, isRefund: Bool) -> Double? {
